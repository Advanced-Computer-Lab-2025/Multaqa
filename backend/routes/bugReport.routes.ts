...
import {
  IBugReportResponse,
  IUpdateBugReportStatusResponse,
} from "../interfaces/responses/bugReportResponses";
...
async function createBugReport(
  ...
  res: Response<IBugReportResponse>
) {
  try {
    ...
    const newBugReport = await bugReportService.createBugReport(
      bugReportData,
      userId
    );
    res.json({
...
async function updateBugReportStatus(
  req: Request,
  res: Response<IUpdateBugReportStatusResponse>
) {
  try {
