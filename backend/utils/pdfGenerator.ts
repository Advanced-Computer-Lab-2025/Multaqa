...
export class pdfGenerator {
  static generateCertificatePDF(data: CertificateData): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.rect(0, 0, width, height).fill("#f0f4ff");
      ...
      doc.circle(80, 80, 60).fill(colors.secondary);
      doc.circle(50, 120, 35).fill(colors.lightBlue);

      // Decorative circles - top right (purple shades)
      doc.circle(width - 80, 60, 45).fill(colors.primary);
      doc.circle(width - 120, 100, 30).fill(colors.accent);
      doc.circle(width - 60, 110, 25).fill(colors.lightPurple);

      // Decorative circles - bottom left (purple-blue mix)
      doc.circle(90, height - 90, 50).fill(colors.accent);
      doc.circle(140, height - 70, 35).fill(colors.lightPurple);

      // Decorative circles - bottom right (blue shades)
      doc.circle(width - 100, height - 100, 55).fill(colors.darkBlue);
      doc.circle(width - 60, height - 140, 40).fill(colors.secondary);
      doc.circle(width - 140, height - 60, 30).fill(colors.lightBlue);
      ...
      // Shadow
      doc.rect(cardX + 5, cardY + 5, cardWidth, cardHeight).fill("#00000010");

      // White card
      doc.rect(cardX, cardY, cardWidth, cardHeight).fill("#FFFFFF");

      // Bluish purple border accent (left side)
      doc.rect(cardX, cardY, 8, cardHeight).fill(colors.primary);

      // Top decorative line
      doc
        .moveTo(cardX + 60, cardY + 50)
      ...
      doc
        ...
        .fillColor(colors.textSecondary)
        .text(
          `Held from ${formatDate(startDate)} to ${formatDate(endDate)}`,
          0,
          cardY + 355,
          {
            align: "center",
