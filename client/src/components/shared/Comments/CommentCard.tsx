...
export default function CommentCard({
  ...
  isDeleting = false,
  isBlocking = false,
  isFlaggingSafe = false,
}: CommentCardProps) {
  const { reviewer, eventName, rating, createdAt, flaggedForToxicity } =
    comment;
    ...
    <CardContainer>
      ...
      <CardHeader>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="Flag as Safe" arrow>
            <span>
              <ActionButton
                actionType="safe"
                onClick={() => onFlagSafe(comment.eventId, reviewer._id)}
                disabled={isFlaggingSafe}
                size="small"
              >
                <Flag size={18} />
              </ActionButton>
