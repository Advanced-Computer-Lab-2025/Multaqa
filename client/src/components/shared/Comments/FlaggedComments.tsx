...
import { fetchFlaggedComments, deleteComment, blockUser } from "./utils";
...
export default function FlaggedComments() {
    ...
    const handleConfirmDelete = async () => {
        ...
        const success = await deleteComment(confirmModal.eventId, confirmModal.userId);
        if (success) {
            setComments((prev) =>
                prev.filter(
                    (c) =>
                        !(c.eventId === confirmModal.eventId && c.reviewer._id === confirmModal.userId)
                )
            );
        }
