...
export const SentimentBadge = styled(Box)<{ severity: "low" | "medium" | "high" }>(
    ({ severity }) => ({
        display: "flex",
        alignItems: "center",
        gap: "6px",
...
export const ActionButton = styled(IconButton)<{ actionType?: "delete" | "block" }>(
    ({ theme, actionType }) => ({
        padding: "8px",
        borderRadius: "8px",
        transition: "all 0.2s ease",
        color: actionType === "delete" ? theme.palette.error.main : theme.palette.warning.main,
...
export const ToxicityScoreBadge = styled(Box)<{ score: number }>(({ score }) => ({
    display: "inline-flex",
    alignItems: "center",
    gap: "4px",
    padding: "4px 10px",
    ...
    fontSize: "11px",
    fontWeight: 600,
    backgroundColor: score >= 0.7 ? "rgba(239, 68, 68, 0.15)" : score >= 0.4 ? "rgba(245, 158, 11, 0.15)" : "rgba(34, 197, 94, 0.15)",
