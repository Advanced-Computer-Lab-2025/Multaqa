...
import { ConferenceViewProps, WorkshopViewProps, BazarViewProps, BoothViewProps } from './Event/types';
...
type Event = ConferenceEvent | WorkshopEvent | BazaarEvent | BoothEvent | TripEvent;
...
const mockEvents: Event[] = [
  ...
  },
  {
    ...
    description: 'Exploring sustainable solutions for a greener future through technology.',
    agenda: 'Opening keynote on climate change\nPanel: Renewable energy solutions\nWorkshop: Sustainable coding practices',
    details: {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter(event => {
        ...
        if (event.type === EventType.BOOTH) {
          return event.company.toLowerCase().includes(query) ||
        ...
        return ('name' in event && event.name.toLowerCase().includes(query)) ||
               ('description' in event && event.description.toLowerCase().includes(query)) ||
               Object.values(event.details).some(value => 
                 value.toString().toLowerCase().includes(query)
               );
      });
